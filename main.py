...
def lendoArquivoPendrive(drive_path, filename, ns_value):
    """
    Lê o conteúdo de um arquivo de texto de um pendrive, decodifica as instruções,
    executa os algoritmos de escalonamento e escreve os resultados em um novo arquivo.

    Args:
        drive_path (str): O caminho base para a pasta de trabalho no pendrive (ex: 'D:/Trabalho').
        filename (str): O nome do arquivo .txt a ser lido (ex: 'TESTE-01.txt').
        ns_value (str): O número formatado do arquivo de teste (ex: '01', '02'), usado para nomear o arquivo de saída.

    Returns:
        str: Uma mensagem indicando o status da operação (sucesso ou erro).
    """
    ...
    # Inicia um bloco try-except para tratar possíveis erros durante a leitura e processamento do arquivo.
    try:
        ...
        # Abre o arquivo de entrada em modo de leitura ('r') com codificação UTF-8.
        with open(file_path, 'r', encoding='utf-8') as f:
            ...
            # 'linhaNum' é o número da linha e 'linha' é o conteúdo da linha.
            for linhaNum, linha in enumerate(f, 2): # Começa a enumerar a partir da linha 2
                # Remove espaços em branco (incluindo quebras de linha) do início e fim da linha.
                ...
                # Converte as partes para inteiros, representando (chegada, burst).
                # Adiciona um ID de processo único (linhaNum - 1) para cada processo.
        ...
        # Passa o dicionário de resultados, o valor ns e o caminho do drive.
        escrever(resultados_algoritmos, ns_value, drive_path)
        # Retorna uma mensagem de sucesso indicando que a decodificação foi concluída.
        return f"Processamento de {filename} concluído com sucesso."
    
    # Captura a exceção FileNotFoundError se o arquivo não for encontrado.
    except FileNotFoundError:
        # Retorna uma mensagem de erro específica para arquivo não encontrado.
    ...
    # Captura qualquer outra exceção que possa ocorrer durante a execução.
        ...
        # Retorna uma mensagem de erro genérica com detalhes da exceção.
        return f"Ocorreu um erro ao ler ou processar o arquivo {filename}: {e}"

# Bloco que é executado apenas quando o script é executado diretamente (não importado como módulo).
if __name__ == "__main__":
    
    # Define o caminho base do pendrive. (Exemplo: em sistemas Windows, pode ser 'D:\TESTES').
    # Em sistemas Linux/macOS, pode ser um caminho como '/media/usuario/PENDRIVE'.
    Caminho_pendrive = 'D:\TESTES'

    # Itera de 1 a 10 para processar 10 arquivos de teste (TESTE-01.txt a TESTE-10.txt).
    for i in range(1, 11):
        # Formata o número 'i' com dois dígitos, preenchendo com zero à esquerda se necessário (ex: 1 -> '01').
        ns = str(i).zfill(2)
